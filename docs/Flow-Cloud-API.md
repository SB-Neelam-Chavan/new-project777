---
stoplight-id: ygky79ax0urw5
---

# Cloud API for eBike Flow app
_Cloud API for eBike Flow app_ provides riders read-only access to their data, collected via the eBike Flow app. Furthermore it empowers OEMs to upload bike model information to be served by the eBike Flow app.
...
- In order to make successful API requests riders need a valid OpenID Connect Access Token (JWT), which must be obtained through SingleKey ID using the `Authorization Code OAuth 2 Flow`. Please refer to security scheme `OAuth 2.0 (SingleKeyID)` for details.
...
- In order to make successful API requests partners need a valid OpenID Connect Access Token (JWT), using the `Client Credentials OAuth 2 Flow`. Please refer to security scheme `OAuth 2.0 (Client)` for details.
